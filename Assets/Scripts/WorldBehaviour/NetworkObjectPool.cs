...
public class NetworkObjectPool : NetworkBehaviour
{
    ...
    private readonly Dictionary<GameObject, Transform> m_PrefabParents = new();
    ...
    {
        if (!m_PooledObjects.TryGetValue(prefab, out var pool))
        {
            Debug.LogError($"[NetworkObjectPool] Prefab {prefab.name} was not registered in pool.");
            return null;
        }
        var networkObject = pool.Get();
    ...
    public NetworkObject SpawnFromPool(GameObject prefab, Vector3 position, Quaternion rotation)
    {
        if (!m_PooledObjects.TryGetValue(prefab, out var pool))
        {
            Debug.LogError($"[Pool] Prefab {prefab.name} is not registered.");
            return null;
        }
    ...
    {
#if UNITY_EDITOR
        if (!m_PrefabParents.TryGetValue(prefab, out var parent))
        {
            GameObject fallbackParent = new GameObject($"{prefab.name} Pool (Client)");
            fallbackParent.transform.SetParent(transform);
            parent = fallbackParent.transform;
            m_PrefabParents[prefab] = parent;
        }
#endif
        ...
        {
#if UNITY_EDITOR
            var go = Instantiate(prefab, m_PrefabParents[prefab]);
